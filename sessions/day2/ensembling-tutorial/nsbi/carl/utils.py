...
def load_results_wifi(run_dir, run_name = 'carl'):
    ...
    for version in versions:
        checkpoint_dir = os.path.join(logs_dir, version, 'checkpoints')
        checkpoints = [f for f in os.listdir(checkpoint_dir) if re.match(r'epoch=\d+-val_loss=.+\.ckpt', f)]
        if not checkpoints:
            raise FileNotFoundError(f"No checkpoints found in {checkpoint_dir}")
        ckpt_path = max(checkpoints, key=lambda f: int(re.search(r'epoch=(\d+)', f).group(1)))
    ...
    return (events_num_val, events_num_test), (events_denom_val, events_denom_test), scaler, ckpts
